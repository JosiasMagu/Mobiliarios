        ...
        <div className="p-5 grid gap-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="rounded-lg ring-1 ring-slate-200 p-3">
              ...
              <div className="text-xs text-slate-500">
                Gasto total {currency(orders.reduce((a: number, o: Order) => a + o.total, 0))}
              </div>
            </div>
          </div>
              ...
              <ul className="text-sm grid gap-1">
                {addresses.map((a: Address) => (
                  <li key={a.id} className="flex items-center justify-between">
                    <span>
                      {a.street} — {a.city}{a.state ? `, ${a.state}` : ""} {a.zip ? `(${a.zip})` : ""}
                    </span>
                  </li>
                ))}
              </ul>
          ...
          <div className="rounded-lg ring-1 ring-slate-200 overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {orders.length === 0 ? (
                  <tr><td colSpan={4} className="px-4 py-6 text-center text-slate-500">Sem pedidos.</td></tr>
                ) : orders.map((o: Order) => (
                  <tr key={o.id} className="border-t border-slate-200/60">
                    ...
                    <td className="px-4 py-2">
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium
                        ${o.status === "delivered" ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100"
                          : o.status === "paid" ? "bg-blue-50 text-blue-700 ring-1 ring-blue-100"
                          : o.status === "cancelled" ? "bg-rose-50 text-rose-700 ring-1 ring-rose-100"
