...
function normalizeOrder(raw: any): Order {
  ...
  return {
    ...
    items: Array.isArray(raw.items) ? raw.items.map((i: any) => ({
      productId: Number(i.productId), name: String(i.name), price: Number(i.price), qty: Number(i.qty), image: i.image
    })) : [],
    subtotal: Number(raw.subtotal ?? 0),
    shippingMethod: down(raw.shippingMethod) as ShippingMethod,
    shippingCost: Number(raw.shippingCost ?? 0),
    total: Number(raw.total ?? 0),
    payment: { method: down(raw.paymentMethod ?? raw.payment?.method) as PaymentKind, reference: raw.payment?.reference, txRef: raw.payment?.txRef },
...
export async function createOrder(payload: {
  ...
  const full = await httpGet<any>(`/api/orders/${raw?.id ?? raw}`);
